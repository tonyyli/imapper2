...
"""
Tools to convert a real-valued function to an intensity map
"""
...
def real_to_fourier(func, x, y, z):
    ...
    if not (is_evenly_spaced(x) and is_evenly_spaced(y) and is_evenly_spaced(z)):
...
def f3d_to_fcylavg(func, x, y, z, bins=None, log=False):
    ...
    rmid_prp    = bin_centers(rcylbins[0])
    rmid_par    = bin_centers(rcylbins[1])
